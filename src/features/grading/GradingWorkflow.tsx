...
function VisionItemCard({ item }: { item: VisionValidationItem; key?: string }) {
  return (
    <article className="grid gap-3 border border-slate-200 bg-white p-3 dark:border-zinc-800 dark:bg-zinc-950">
      ...
      {item.evidenceUnits?.length ? (
        <div className="grid grid-cols-2 gap-2">
          {item.evidenceUnits.map(unit => (
          ...
          <div className="grid gap-4 xl:grid-cols-[250px_minmax(0,1fr)]">
            ...
            <div className="space-y-4">
              <div className="grid gap-3 lg:grid-cols-3">
                ...
                <section className={`${panelClass} min-h-80 p-4`}><div className="flex items-center justify-between"><h3 className="flex items-center gap-2 text-sm font-black"><ScanLine className="h-4 w-4 text-emerald-700" />OCR 文本</h3><span className={`rounded-xl px-2 py-1 text-xs font-bold ${selectedSample.needsTeacherReview || selectedSample.ocrConfidence < ocrHumanReviewThreshold ? 'bg-rose-100 text-rose-800' : 'bg-slate-100 text-slate-600'}`}>{selectedSample.needsTeacherReview ? '待核验 · ' : ''}{Math.round(selectedSample.ocrConfidence * 100)}%</span></div><textarea value={editedOcr} onChange={event => setEditedOcr(event.target.value)} rows={9} className={`${inputClass} mt-4 resize-none leading-7`} />{selectedSample.needsTeacherReview && selectedSample.lunaReviewText ? <div className={`mt-3 rounded-lg border p-3 text-xs leading-5 ${selectedSample.recognitionConflict ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-slate-200 bg-slate-50 text-slate-700'}`}><strong>{selectedSample.recognitionConflict ? 'PaddleOCR 与视觉复核不一致' : 'Luna 视觉复核'}</strong><p className="mt-1">{selectedSample.lunaReviewText}</p></div> : null}<button type="button" onClick={saveOcrCorrection} className="mt-3 rounded-2xl border border-slate-200 px-3 py-2 text-xs font-bold dark:border-zinc-700">保存 OCR 修正</button></section>
