  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="group w-full overflow-hidden border border-slate-200 bg-white text-left transition-colors hover:border-emerald-500 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        {evidence.isPartialBlock ? <p className="border-t border-sky-200 bg-sky-50 px-3 py-2 text-[11px] font-bold text-sky-800 dark:border-sky-900 dark:bg-sky-950/20 dark:text-sky-200">这是引用文字所在的 OCR 块原图，可能同时包含其他题目；准确引用见上方文字。</p> : null}
      ...
      {open ? (
        <div className="fixed inset-0 z-[70] flex flex-col bg-slate-950/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`${label}来源原图`}>
          ...
          <div className="mx-auto flex min-h-0 w-full max-w-6xl flex-1 flex-col items-center overflow-auto rounded-b-lg bg-slate-200 p-4 sm:p-8 dark:bg-zinc-950"><div className="aspect-[3/4] w-[min(680px,75vw)] flex-none overflow-hidden bg-white shadow-2xl transition-transform" style={{ transform: `scale(${zoom}) rotate(${rotation}deg)` }}><EvidencePage evidence={evidence} expanded /></div>{evidence.ocrText ? <div className="mt-4 w-full max-w-3xl rounded-xl bg-white p-4 text-sm leading-6 text-slate-700 shadow dark:bg-zinc-900 dark:text-slate-200"><strong className="block text-xs text-emerald-700">本题引用原文</strong><p className="mt-2 whitespace-pre-wrap">{evidence.ocrText}</p></div> : null}</div>
          <span className="pointer-events-none absolute bottom-7 left-1/2 flex -translate-x-1/2 items-center gap-1 rounded-lg bg-slate-950/75 px-3 py-1.5 text-xs text-white"><FileSearch className="h-3.5 w-3.5" />{evidence.isPartialBlock ? '引用所在 OCR 块' : '来源区域'} {Math.round(evidence.boundingBox.x * 100)}%, {Math.round(evidence.boundingBox.y * 100)}%</span>
        </div>
      ) : null}
    </>
  );
