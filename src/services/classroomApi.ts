...
export const getClassroomLayout = async (classId: string) => {
  const response = await apiFetch(`/api/classes/${encodeURIComponent(classId)}/classroom-layout`);
...
export const saveClassroomLayout = async (layout: ClassroomLayout) => {
  const response = await apiFetch(`/api/classes/${encodeURIComponent(layout.classId)}/classroom-layout`, {
    method: 'PUT',
...
export const exportClassroomLayout = async (classId: string, className: string, includeStudentNo: boolean) => {
  const response = await apiFetch(`/api/classes/${encodeURIComponent(classId)}/classroom-layout/export?includeStudentNo=${includeStudentNo}`);
  if (!response.ok) throw new Error(await readErrorCode(response));
