...
test('roster data is isolated by workspace context', () => {
  ...
  runWithWorkspace(first, () => {
    getRosterDatabase().prepare(`INSERT INTO classes (id, name, grade, term, head_teacher, chinese_teacher, textbook_version, default_submit_time, status, created_at, updated_at) VALUES ('same-id', '一班', '七年级', '2026', '', '', '', '', 'active', 'now', 'now')`).run();
  });
  runWithWorkspace(second, () => {
    ...
    getRosterDatabase().prepare(`INSERT INTO classes (id, name, grade, term, head_teacher, chinese_teacher, textbook_version, default_submit_time, status, created_at, updated_at) VALUES ('same-id', '二班', '七年级', '2026', '', '', '', '', 'active', 'now', 'now')`).run();
  });
