...
export const buildTrialAnswerEvidence = (item: VisionValidationItem) => {
  ...
  const lunaFallback = !item.selectedOption && !paddleAnswer && Boolean(item.lunaText.trim());
  return {
    displayNo: item.displayNo,
    answer: item.selectedOption || paddleAnswer || item.lunaText,
    answerSource: item.selectedOption ? 'choice-vision' : paddleAnswer ? 'paddle' : item.lunaText ? 'luna-fallback' : 'none',
    paddleAnswer,
    ...
    needsReview: item.needsReview || item.locationStatus !== 'located' || recognitionConflict || lunaFallback
  };
};
...
export const buildTrialGradingPrompt = (request: TrialGradingRequest, submissions: unknown[]) => [
  ...
  'answer 是系统已经选定的“评分采用文本”，是本次实际评分输入；answerSource 说明其来自 choice-vision、paddle、luna-fallback、teacher-corrected 或 none。不得因为 paddleAnswer 为空而忽略非空的 answer。',
  'answerSource=luna-fallback 时仍须依据 answer 给出暂定数字分，同时 needsTeacherReview=true；lunaReview 是视觉复核文本，不得在 answer 非空时声称缺少学生作答。',
  'crossedOutText 是 Luna 从图片确认的划掉内容，评分时必须从主证据中排除。existingMarkings 是已有分数、勾叉或批注，必须忽略。',
  'recognitionConflict=true 表示两路证据有实质差异：仍须依据 answer 给出暂定分数，同时 needsTeacherReview=true，并在 reason 中说明冲突，不得因为冲突直接返回 null。',
  '只要 answer 非空且评分依据足够，score 必须是 0 到 fullScore 之间的数字；只有 answer 为空或评分依据本身不足时才可返回 null。',
    ...
    const submissions = materials.flatMap(material => {
      ...
      return [{
        ...
        recognizedAnswers: (getVisionValidationResult(taskId, material.id)?.items ?? []).filter(item => requestedQuestionNos.has(item.displayNo)).map(item => {
          ...
          return corrected === undefined ? evidence : { ...evidence, answer: corrected, answerSource: 'teacher-corrected', paddleAnswer: corrected, recognitionConflict: false, needsReview: false };
        })
      }];
    });
