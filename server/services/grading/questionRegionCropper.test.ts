...
test('uses OCRv6 question anchors while preserving the VL completeness candidate', async () => {
  const page = await makePage(1);
  const artifact: PaddleParserArtifact = {
    model: 'PaddleOCR-VL-1.6',
    ocrModel: 'PP-OCRv6',
    pages: [{ pageNumber: 1, prunedResult: { width: 800, height: 1000, parsing_res_list: [
      { block_label: 'text', block_content: '8. 第八题长答案\n9. 第九题完整答案', block_bbox: [70, 180, 390, 410], block_id: 1 },
      { block_label: 'text', block_content: '10. 第十题答案', block_bbox: [70, 450, 390, 490], block_id: 2 }
    ] } }],
    ocrPages: [{ pageNumber: 1, width: 800, height: 1000, lines: [
      { text: '8. 第八题答案', confidence: 0.96, boundingBox: [70, 180, 250, 205] },
      { text: '9. 第九题答案', confidence: 0.98, boundingBox: [70, 300, 250, 325] },
      { text: '第九题第二行', confidence: 0.94, boundingBox: [70, 350, 300, 375] },
      { text: '10. 第十题答案', confidence: 0.97, boundingBox: [70, 450, 250, 475] }
    ] }]
  };
  try {
    const [region] = await createVisionLocatedRegions(
    ...
    assert.ok(region.region.y < 300, JSON.stringify(region.region));
    assert.ok(region.region.y + region.region.height > 450, JSON.stringify(region.region));
    assert.match(region.ocrV6Text, /^9\. 第九题答案\n第九题第二行$/);
    assert.match(region.vlText, /^9\. 第九题完整答案$/);
    assert.equal(region.paddleText, region.vlText);
  } finally {
    await rm(page.sourceImagePath, { force: true });
    await rm(path.resolve('var/uploads/validation', taskId), { recursive: true, force: true });
  }
...
test('keeps an OCRv6-anchored question when VL merges it into the previous block', async () => {
  const page = await makePage(1);
  const artifact: PaddleParserArtifact = {
    model: 'PaddleOCR-VL-1.6',
    ocrModel: 'PP-OCRv6',
    pages: [{ pageNumber: 1, prunedResult: { width: 800, height: 1000, parsing_res_list: [
      { block_label: 'text', block_content: '8. 第八题长答案\n第九题答案未保留题号', block_bbox: [70, 180, 390, 410], block_id: 1 },
      { block_label: 'text', block_content: '10. 第十题答案', block_bbox: [70, 450, 390, 490], block_id: 2 }
    ] } }],
    ocrPages: [{ pageNumber: 1, width: 800, height: 1000, lines: [
      { text: '8. 第八题答案', confidence: 0.96, boundingBox: [70, 180, 250, 205] },
  ...
  try {
    ...
    assert.equal(region.locationStatus, 'located');
    assert.equal(region.evidenceUnits.length, 1);
    assert.equal(region.evidenceUnits[0].region.y, region.region.y);
    assert.equal(region.needsFocusedOcr, true);
  } finally {
    await rm(page.sourceImagePath, { force: true });
    await rm(path.resolve('var/uploads/validation', taskId), { recursive: true, force: true });
