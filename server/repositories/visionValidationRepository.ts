...
export const NON_CHOICE_RECOGNITION_VERSION = 13;
...
export const deleteVisionValidationForAssets = (taskId: string, assetIds: string[]) => { const current = store(); let changed = false; for (const assetId of assetIds) changed = current.values.delete(keyFor(taskId, assetId)) || changed; if (changed) current.persist(); };
