...
test('persists authoritative roster relationships and constraints', () => {
  assert.deepEqual(repository.listClasses().map(item => item.id), ['c5']);
  ...
  const fiveClass = repository.listClasses().find(item => item.id === 'c5');
  ...
  const match = repository.matchSubmissions('c5', ['5001', '5001', '9999']);
  ...
  repository.updateClass('c5', { representatives: [second.studentId] });
  assert.equal(repository.findStudentByNo('c5', '5001')?.isRepresentative, false);
  assert.equal(repository.findStudentByNo('c5', '5002')?.isRepresentative, true);
