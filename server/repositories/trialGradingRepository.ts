...
export const deleteTrialGradingForAssets = (taskId: string, assetIds: string[]) => { const current = store(); const result = current.values.get(taskId); if (!result) return; const removed = new Set(assetIds); const samples = result.samples.filter(sample => !sample.sourceAssetId || !removed.has(sample.sourceAssetId)); if (samples.length === result.samples.length) return; current.values.set(taskId, { ...result, samples, createdAt: new Date().toISOString() }); current.persist(); };
