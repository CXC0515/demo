...
### 14.1 后续 TODO：COS 大文件上传

- 仅在试用中频繁出现超过 80 MiB 的资料时，再进入详细设计和实施审批。
- 评估使用腾讯云 COS 分片直传，使大文件上传支持断点续传，并将目标上限提高到 200 MiB 以上。
- 设计前必须明确私有访问、临时签名、租户权限、OCR 文件链路、引用一致性、迁移、备份和回滚方案。
- 当前阶段资料仍存放在服务器持久磁盘；已购买的 100G COS 优先作为异地备份目标，不作为产品权威数据源。
- 本 TODO 只是后续议题，不改变本阶段“不引入对象存储”的实施边界。
...
| v1.4 补记 | 2026-09-10 | 待后续设计 | 记录 COS 大文件分片直传 TODO、触发条件及当前仅作为异地备份目标的边界。 |
