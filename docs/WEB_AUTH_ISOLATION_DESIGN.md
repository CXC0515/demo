...
3. 教师资料和工作区名称采用注册时实际填写的姓名，不沿用邀请备注名。
4. 普通教师注册成功后产生新的空工作区；迁移数据只存在于首个所有者候选工作区，不复制给新用户。
5. 注册接口不创建会话，前端返回普通登录页并预填邮箱；用户再次提交邮箱和密码后才建立会话。
6. 管理员生成一次性密码重置链接，经 Agent Mail 人工发送；链接 30 分钟有效。
7. 退出后原受保护文件 URL 立即失去访问权限。
...
| 版本 | 日期 | 状态 | 修改概要 |
| --- | --- | --- | --- |
| v1.0 | 2026-09-07 | 已被 v1.1 取代 | 记录邀请认证、工作区物理隔离、文件授权、CSRF/CSP、管理员边界与已知取舍。 |
| v1.1 | 2026-09-08 | 当前 | 明确注册与登录分为两个动作、注册阶段不创建会话、注册姓名为资料权威值，并重申新教师工作区为空。 |
