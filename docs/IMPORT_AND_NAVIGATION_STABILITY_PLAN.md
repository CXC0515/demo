...
- 当前版本：v1.2
- 日期：2026-09-10
- 状态：PR #16 已合并并部署，待三个试用账号完成真实业务验收
- 代码基线：`origin/main@52754d1`
...
| 版本 | 日期 | 概要 |
| --- | --- | --- |
| v1.2 | 2026-09-10 | PR #16 已合并部署；升级前产品快照、母文件夹构建测试、LaunchAgent 重启、浏览器登录页和公网静态资源边界均通过。 |
...
- PR #16 合并提交为 `52754d1`；母文件夹保留文档历史后普通合并为 `9584d7c`。部署前快照 `product-2026-09-10T04-07-32-pre-pr16` 包含 192 个文件和 5 个 SQLite，清单、哈希及数据库完整性校验通过。
- 正式应用重启后 PID 更新为 71615，回环和公网 ready 均为 200，AI/PaddleOCR 配置为 ready；本地与公网主资源均为 `index-BuhYsX85.js`。公网 `/login` 为 200/no-store，图谱 chunk 为 JavaScript/immutable，模拟旧 chunk 为 404/no-store，未登录业务 API 为 401。
- 浏览器已确认部署后的登录页正常渲染。尚未代替教师写入真实课表、座位或知识数据；课表实际 AI 延迟、班级可视化二次进入和登录后图谱交互由真实账号下一轮验收。
- 公网检查期间出现一次 Cloudflare TLS 瞬断，立即重试恢复；与既有 Tunnel/本机网络波动边界一致，不能由本次页面修复消除。
