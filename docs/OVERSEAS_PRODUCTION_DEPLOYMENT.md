...
- 迁移前本地权威数据来源：`/Users/cxc/Projects/DEMO/var-product`；2026-09-16 切换后，当前生产权威数据为服务器 `/var/lib/teacher-dashboard/product-2026-09-16-r2`。本地目录仅保留切换前证据，不再接受生产写入，也不能覆盖服务器后续新增数据。
- 用户已确认该产品数据可以存储到境外。
...
| 2026-09-23 | 已完成 | PR #46 材料池发布前创建并验证手工在线恢复点 `product-2026-09-23-pre-material-pool`（1,285 个文件、7 个 SQLite）；发布 `54b3159` 后三个工作区完成资源库 schema v9 迁移，完整性、外键和旧资料引用检查通过。保留 `296443c` 发布目录，未改权威数据根。 |
...
| v1.1 | 2026-09-16 | 已被 v1.2 取代 | 记录服务器安装、数据恢复修复、DNS、HTTPS、防火墙、自动备份和公网首屏验收结果。 |
| v1.2 | 2026-09-20 | 历史 | 澄清本地 `var-product` 仅为迁移前来源，当前生产权威数据为服务器 `product-2026-09-16-r2`。 |
| v1.3 | 2026-09-23 | 当前 | 记录材料池数据库迁移前恢复点、生产发布、三个工作区数据校验和回滚保留版本。 |
