...
| 2026-09-23 | 已完成 | PR #48 材料池移动端布局修正发布为 `88bf9f5`；本地 lint/build、服务器 build、回环和公网首页/live/ready 通过。保留 `54b3159` 发布目录；未改生产数据或数据库结构。真实账号手机操作待用户复测。 |
...
| v1.3 | 2026-09-23 | 历史 | 记录材料池数据库迁移前恢复点、生产发布、三个工作区数据校验和回滚保留版本。 |
| v1.4 | 2026-09-23 | 当前 | 记录 PR #48 移动布局发布、健康检查、回滚版本与真实手机待验收项。 |
